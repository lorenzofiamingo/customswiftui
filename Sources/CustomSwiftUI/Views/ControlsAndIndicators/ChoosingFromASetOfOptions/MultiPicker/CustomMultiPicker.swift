...
///     }
///
///     @State private var selectedFlavors: Set<Flavor> = Set()
///
...
///
///     List {
///         CustomMultiPicker("Flavor", selection: $selectedFlavors) {
...
///
///     CustomPicker("Flavor", selection: $selectedFlavors) {
...
///     }
///
///     @State private var suggestedToppings: Set<Topping> = Set()
///
...
///
///     List {
///         CustomPicker("Flavor", selection: $suggestedToppings) {
...
///     }
///
/// When the user selects chocolate, the picker adds to `suggestedToppings`
/// the value in the associated tag:
///
...
///
///     VStack {
///         CustomMultiPicker("Flavor", selection: $selectedFlavor) {
///             CustomForEach(Flavor.allCases) { flavor in
///                 Text(flavor.rawValue.capitalized)
///             }
///         }
///         CustomMultiPicker("Topping", selection: $selectedTopping) {
...
public struct CustomMultiPicker<Label, SelectionValue, Content>: View where Label: View, SelectionValue: Hashable, Content: CustomView {
    ...
    @Environment(\.customMultiPickerStyle) private var style
    ...
    public var body: some View {
        AnyView(style.makeMultiPicker(configuration: configuration))
    }
}
...
@available(iOS 13.0, macOS 10.15, tvOS 13.0, watchOS 6.0, *)
extension CustomMultiPicker where Label == Text {
    ...
    public init(_ titleKey: LocalizedStringKey, selection: Binding<Set<SelectionValue>>, @CustomViewBuilder content: () -> Content) {
    ...
    /// ``init(_:selection:content:)`` instead.
    public init<S>(_ title: S, selection: Binding<Set<SelectionValue>>, @CustomViewBuilder content: () -> Content) where S : StringProtocol {
        self.init(selection: selection, content: content) {
            Text(title)
        }
    }
